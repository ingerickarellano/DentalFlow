...
const GestionTrabajos: React.FC<GestionTrabajosProps> = ({ onBack }) => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Cargar todos los datos del usuario autenticado
  ...
  // ORGANIZAR POR CLÍNICA -> PACIENTE (Cada paciente tiene sus propios trabajos)
  ...
  const getEstadoStyle = (estado: string): React.CSSProperties => {
    ...
    switch (estado) {
      case 'pendiente':
        return { ...baseStyle, backgroundColor: '#fef3c7', color: '#92400e', border: '2px solid #fbbf24' };
      case 'produccion':
        return { ...baseStyle, backgroundColor: '#dbeafe', color: '#1e40af', border: '2px solid #60a5fa' };
      case 'terminado':
        return { ...baseStyle, backgroundColor: '#d1fae5', color: '#065f46', border: '2px solid #34d399' };
      case 'entregado':
        return { ...baseStyle, backgroundColor: '#e5e7eb', color: '#374151', border: '2px solid #9ca3af' };
      default:
        return { ...baseStyle, backgroundColor: '#fef3c7', color: '#92400e', border: '2px solid #fbbf24' };
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.filtrosContainer}>
        <h3 style={{ fontSize: '1.5rem', fontWeight: '600', marginBottom: '1.5rem' }}>🔍 Filtros de Búsqueda</h3>
      ...
      {cargando && trabajos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem' }}>
          <div style={{ fontSize: '1.5rem', color: '#64748b' }}>Cargando trabajos...</div>
        </div>
        ...
        Object.entries(trabajosPorClinicaYPaciente).map(([clinicaId, pacientesObj]) => {
          ...
          // Calcular trabajos pendientes para esta clínica
          const trabajosPendientes = trabajos.filter(t => 
            t.clinica_id === clinicaId && 
            (t.estado === 'pendiente' || t.estado === 'produccion')
          ).length;
                                  ...
                                  <div key={index} style={styles.servicioItem}>
                                    <div>
                                      ...
                                      {servicioTrabajo.nota_especial && (
                                        <div style={{ fontSize: '0.75rem', color: '#f59e0b', fontStyle: 'italic', marginTop: '0.25rem' }}>
                                          📝 {servicioTrabajo.nota_especial}
                                        </div>
                                      )}
                                    </div>
